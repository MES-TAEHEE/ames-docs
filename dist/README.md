...
PDA 구조와 감사 사용자 매핑표를 포함한 테이블 174개, 컬럼 2,470개, 프로시저 39개, 외래키 28개,
...
감사 컬럼 변경 전에는 `rebuild_db.sh` 후속 SQL 30개도 검증했습니다. 이후
`CreatedBy`, `ModifiedBy`, `ApprovedBy`, `RequestedBy` 330개 컬럼을 varchar(20)으로
통일했으므로, 기존 긴 사용자 ID·작업 식별자를 쓰는 소스/프로시저/후속 시드는
20바이트 이하 코드로 입력을 변경해야 합니다. 이 매핑은 자동으로 수행되지 않습니다.
기존 DB 변환은 `migrate_audit_actor_varchar20.sql`을 사용하며, 원문은
`SYS_AuditActorMap`에 보존합니다. 신규 DB의 매핑표는 비어 있습니다.
