...
- **로그인 2곳, 처리는 하나**(`Services/WebSignIn.cs`, 09-24): 어느 화면에서 로그인하든 **계정 종류로 갈린다** — `ExternalCustomer` 계정은 내부 `Account/Login` 에서 로그인해도 외부 쿠키를 받아 외부 화면(`ReturnUrl` 이 `/portal` 이 아니면 `PortalAuth.HomePath`)으로, 그 밖의 계정은 `/portal/login` 에서 로그인해도 내부 쿠키를 받아 내부로 간다(이메일 확인·승인·5회 잠금 등 내부 규칙 그대로, ID 기억 쿠키는 내부 화면에서 내부 로그인일 때만). 두 로그인 경로는 `Program.cs` 미들웨어가 요청 사용자를 익명으로 바꿔 처리한다 — 위조 방지 토큰이 화면을 연 시점의 사용자에 묶여, 화면을 열어 둔 채 다른 탭에서 로그인하면 첫 POST 가 400(→ `?expired=1`)이 돼 버튼을 두 번 눌러야 했다(09-24). 외부 로그인은 `UserManager.CheckPasswordAsync` + 잠금 카운터로 검증하고 내부 Identity 쿠키를 지운 뒤 **`AmesPortal` 스킴 쿠키 `.AMES.Portal`(경로 `/`)** 을 발급한다. 경로를 `/portal` 로 좁히면 Blazor 회로(`/_blazor`)에 쿠키가 안 실려 로그인 직후 권한 없음이 된다. 로그아웃은 `GET /portal/logout`(외부 쿠키만 삭제). 외부 쿠키로 내부 화면에 막힌 `/unauthorized?ReturnUrl=<내부 경로>` 의 "홈으로" 는 `/portal/logout?to=internal&returnUrl=…` 로 외부 쿠키를 지우고 내부 로그인(로그인 후 원래 화면)으로 보낸다 — ReturnUrl 이 없거나 외부 화면이면 외부 로그인으로, 외부 호스트에서는 `to=internal` 을 무시한다. returnUrl 은 같은 사이트 절대 경로만 받는다(`PortalAuth.IsLocalReturnUrl`).
