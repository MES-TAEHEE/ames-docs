...
IMG 원천 LOT(`PR_ImgLot`)은 `dist/migrate_img_lot.sql` — `migrate_lotno_rule.sql` 뒤에 적용. 이게 없으면 IMG-MAIN 이 5초마다 조회 예외를 띄우고 라벨 발행이 안 된다.
