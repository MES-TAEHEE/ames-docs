    ...
    public void Receipt_then_release_records_history_without_leaving_test_stock()
    {
        ...
        try
        {
            using var cmd = new SqlCommand("""
                DECLARE @Created table (SerialNo varchar(24));
                INSERT dbo.MD_SparePart (SparePartNo,PartNo,PartName,ZoneCode,Slot,OnHandQty,ActiveFlag,CreatedBy)
                VALUES (@Part,@Part,'Rollback-only receipt test','SP_A1','01',0,1,'receipt-test');
                INSERT @Created EXEC dbo.SP_PDA_SP_SERIAL_CREATE @Part,1,'receipt-test';
                DECLARE @Serial varchar(24)=(SELECT TOP (1) SerialNo FROM @Created);
                EXEC dbo.SP_PDA_STOCK_MOVE @Serial,'IN',1,NULL,'receipt-test','Rollback-only receipt';
                EXEC dbo.SP_PDA_STOCK_MOVE @Serial,'OUT',1,NULL,'receipt-test','Rollback-only release';
    ...
    public void Master_quantity_without_receipt_cannot_be_released()
    {
        ...
        try
        {
            using var cmd = new SqlCommand("""
                DECLARE @Created table (SerialNo varchar(24));
                INSERT dbo.MD_SparePart (SparePartNo,PartNo,PartName,ZoneCode,Slot,OnHandQty,ActiveFlag,CreatedBy)
                VALUES (@Part,@Part,'Rollback-only never-received test','SP_A1','01',3,1,'receipt-test');
                INSERT @Created EXEC dbo.SP_PDA_SP_SERIAL_CREATE @Part,1,'receipt-test';
                DECLARE @Serial varchar(24)=(SELECT TOP (1) SerialNo FROM @Created);
                EXEC dbo.SP_PDA_STOCK_MOVE @Serial,'OUT',1,NULL,'receipt-test','Must be blocked';
                """, conn, tx);
            cmd.Parameters.AddWithValue("@Part", part);
