...
/// <summary>
/// IMG-05 Defect entry — pick one of the MD_DefectCode codes for the
...
/// </summary>
public sealed class Img05DefectForm : PopForm
{
    ...
    public Img05DefectForm(PopSessionDto session) : this(session, null, 0) {}

    public Img05DefectForm(PopSessionDto session, WorkOrderDto? wo, int resultId)
    {
        _session  = session;
        _wo       = wo;
        _resultId = resultId;
        Text      = "A-MES POP · IMG-05 Defect";
        ...
        root.Controls.Add(PopShell.BuildTopBar("IMG-05 · Defect", session), 0, 0);
        ...
        try
        {
            ...
            if (pct >= 5)
            {
                ...
                PopServices.Andon.Raise(_session.LineId, equipId: null,
                    triggerSource: "IMG-05", ruleId: "BR-INJ-03", severity: "HIGH", _session.EmployeeNo);
            }
