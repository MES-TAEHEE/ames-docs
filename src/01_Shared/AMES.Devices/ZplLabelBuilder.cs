...
/// <summary>
/// SEOYON 원본 태그와 같은 배치를 4"×2"(812×406 dot @ 203dpi)로 2배 확대한 레이아웃.
...
public static class ZplLabelBuilder
{
    public static string Build(ZplLabel l)
    {
        ...
        sb.AppendLine("^LH0,0");
        sb.AppendLine("^CI28");
        sb.AppendLine($"^FO50,40^BXN,12,200^FD{Zf(l.LotCode)}^FS");
        // Invariant 고정 — 서식의 '/' 는 문화권 날짜구분자로 치환되고(ko → '-'),
        // 비그레고리력 문화권에서는 연도까지 달라진다.
        sb.AppendLine($"^FO290,40^A0N,60,60^FD{l.ProducedAt.ToString("MM/dd/yyyy", CultureInfo.InvariantCulture)}^FS");
        sb.AppendLine($"^FO290,110^A0N,70,70^FD{Zf(l.LotCode)}^FS");
        Field(sb, "^FO280,164^A0N,62,54", l.ColorCode);
        Field(sb, "^FO640,28^A0N,86,68", l.CavityPos);
        Field(sb, "^FO640,146^A0N,86,68", l.PressType);
        sb.AppendLine($"^FO50,260^A0N,100,78^FD{Zf(l.ItemNo)}^FS");
        sb.Append("^XZ");
        return sb.ToString();
    }
