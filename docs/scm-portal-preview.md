...
| PORTAL-005 | 입고 현황 | /portal/receipts |
...
- EOS는 별도 검수 단계 없이 입고로 완료한다. 포탈은 납품 품목의 입고수량을 기준으로 미입고 / 부분입고 / 입고완료를 표시하며, EOS 입고 처리는 변경하지 않는다.
...
### 박스번호 변경
- `migrate_scm_box_number.sql`을 delivery_boxes 마이그레이션 이후 적용. 신규 생성 스키마에도 포함.
- 신규 박스는 BX-{VendorID}-{서버 생성일 yyyyMMdd}-{최소 4자리 순번}. 업체/일자별 0001 시작, 9999 이후에도 자릿수를 늘려 중복 없이 증가.
- SCM_BoxNumberSequence에서 트랜잭션 내 범위를 예약. 취소/무효 박스의 발급번호도 재사용하지 않음.
- SCM_DeliveryBox.IssuedBoxNumber에 신규 번호 저장; BoxNumber는 신규 번호 또는 기존 BOX-{BoxID}로 표시. 기존 번호는 유지. 수량 수정으로 재생성된 박스는 새 번호 사용.

### 포털 납품 메뉴 통합
- PORTAL-003 납품서 관리: /portal/deliveries. 등록 버튼 → 수주 확인된 잔량 발주 선택 → 품목별 등록 → 동일 목록. 상세 수정/취소/출하 확정 및 식별표 연결 유지.
- /portal/delivery-entry는 통합 화면의 호환 주소.
- PORTAL-004 딜리버리 노트 조회·발행: /portal/delivery-notes.
- 기존 설치에는 migrate_scm_portal_delivery_menu.sql 적용; 화면 코드와 기존 권한 연결은 유지하고 이름/주소 갱신. 생성 스키마에도 포함.

### 납기 화면 / 언어 표시
- PORTAL-002는 수주 확인 완료 발주만 납기순 표시. 기본 상태는 미납 발주(발행/입고 진행 및 미입고 수량 존재). 상태 선택으로 마감/취소 이력 조회, 초기화 시 미납 기본 복원.
- 포털 박스/노트/입고 결과/적입량 화면의 한국어 고정 문구를 PortalText로 연결. 영어 UI에서 제목/버튼/안내/상태 라벨/오류 문구 전환. 상태 키는 그대로 유지.

### PORTAL-002 납품 준비 현황
수주 확인 완료 발주를 품목별로 표시. 기본 등록 가능 필터는 발주수량-입고수량-등록 후 미입고수량이 양수인 품목만 표시. 취소된 납품서는 예약 수량에서 제외. 납기순 정렬 및 지연 표시. 행의 납품서 등록 버튼은 PORTAL-003에 OrderNumber를 전달하며, 대상 발주의 현재 권한/확인/잔량을 다시 조회한 후 등록 창을 연다.

## 포탈 입고 현황 명칭 정리
- PORTAL-005 메뉴·화면명을 입고 현황(Receipt Status)으로 통일하고 검수 안내를 제거했다.
- 입고수량이 0이면 미입고, 납품수량 미만이면 부분입고, 전량이면 입고완료다. 기존 읽기 전용 조회와 업체별 접근 제한을 유지한다.
- 화면명 반영: `dist/migrate_scm_portal_receipt_status.sql`. EOS 내부 입고 로직과 업무 데이터는 변경하지 않는다.

## SCM-002 발주 진행 현황
- 수주 확인 여부와 확인일시, 납품서 등록량, 누적 출하량, 미출하 잔량, 입고량을 표시한다. 발주별 합계는 단위별로 구분하고 상세에서는 품목별로 표시한다.
- 취소 납품서는 제외한다. 출하확정일시가 있거나 Shipped/Received 상태인 납품서만 출하량에 포함하며 입고 후에도 출하량은 유지한다.
- 진행 상태는 확인 대기, 수주 확인, 납품 준비, 부분 출하, 출하 완료, 부분 입고, 입고 완료로 표시한다. 작성중/취소는 기존 상태를 유지한다. EOS 입고 로직은 변경하지 않는다.
