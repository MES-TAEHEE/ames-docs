...
SCM-001/002는 기존 WH_PurchaseOrder의 실제 발주를 저장·조회한다. 포털은 실제 발주 조회, 수주 확인, 납품 예정 등록과 조회를 지원한다. 납품서 수정·취소도 DB에 반영한다. 입고·검수 및 출하 확정은 후속 구현이다. SAP 호출은 하지 않는다.
...
- Open/Partial만 확인 가능하다. 외부 계정은 ExternalCustomer 역할과 활성 업체 연결이 필요하며 저장 시 DB에서 재검증한다. 화면 편집 권한도 필요하다.
...
- 발주 잠금/rowversion/요청 ID로 동시 등록, 초과 수량 및 중복 재시도를 막는다. 저장 시 계정 역할과 활성 업체 연결을 재검사한다. 관리자 대리 등록은 현재 관리자 ID를 기록한다.
...
- 납품 예정 상태이며 입고수량/입고 패키지 연결이 없는 경우만 수정·전체 취소할 수 있다. 취소 시 이력은 보존하고 잔량을 복원한다. 출하 확정이 추가되면 그 상태의 수정도 거부한다.
...
7. `dist/migrate_scm_delivery.sql` — 납품 예정 헤더·품목과 누락된 포털 화면 권한
